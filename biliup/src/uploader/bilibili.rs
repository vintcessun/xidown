...
/// [xidown 本地补丁] 只有 `cli` feature 下的 clap 参数解析会用到它，
/// 不加 cfg 的话默认编译就是一段死代码
#[cfg(feature = "cli")]
...
impl BiliBili {
    ...
    async fn archives(&self, status: &str, page_num: u32) -> Result<Value> {
        ...
        // [xidown 本地补丁] 显式指定每页条数：不给的话服务端默认一页只有 10 条，
        // 稿件上千的账号要翻一百多页，光列稿件就要花一分多钟，还容易触发 -702 限流
        let params = [
            ("status", status),
    ...
    pub async fn recent_archives_page(
        ...
        while page_num < plan.to_page {
            page_num = page_num
                .checked_add(1)
                .ok_or_else(|| Kind::Custom("archive page range is too large".into()))?;
            // [xidown 本地补丁] 稿件多的账号要翻几十页，不歇一下会被投稿中心限流(code -702)
